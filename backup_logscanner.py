    ...
    with open(log_file_path, 'r') as log_file:
        ...
        for line in log_file:
            ...
            if not line.startswith('===='):
                ...
                if match:
                    log_timestamp = match.group(1)  # Extract the matched timestamp
                else:
                    log_timestamp = None

            # # Extract the timestamp from the log line (assuming the format "YYYY-MM-DD HH:MM:SS")
            # timestamp_str = line.split(' ')[0] + ' ' + line.split(' ')[1] if line.strip() else None
            # try:
            #     log_timestamp = datetime.strptime(timestamp_str, "%Y-%m-%d %H:%M:%S") if timestamp_str else None
            # except ValueError:
            #     log_timestamp = None

            # If a timestamp range is specified, check if the log's timestamp is within range
            if start_time and log_timestamp and log_timestamp < start_time:
                continue
            if end_time and log_timestamp and log_timestamp > end_time:
                continue

            # Check if any of the search keywords are present in the line
            if any(keyword in line for keyword in search_keywords) or (
                ...
                # Log the matching result to Snowflake
...
def scan_log_directory(directory_path, search_keywords,system_type, start_time=None, end_time=None, use_regex=False,
    ...
    override = input("Do you want to override the saved timestamp? (yes/no): ").strip().lower() == 'yes'

    if override:
        custom_timestamp_str = input("Enter the timestamp to scan from (DD-MM-YYYY HH:MM:%S): ").strip()
        try:
            file_filter_timestamp = datetime.strptime(custom_timestamp_str, "%d-%m-%Y %H:%M:%S")
        except ValueError:
            print("Invalid timestamp format. Using fallback default.")
            file_filter_timestamp = datetime.strptime("01-01-1970 00:00:00", "%d-%m-%Y %H:%M:%S")
    else:
    ...
    with tqdm(total=len(filtered_files), desc="Scanning files", unit="file") as pbar:
        ...
        for filename in filtered_files:
            ...
            if skip_processed_files and is_file_processed(filename, conn):
            ...
            if skip_processed_files:
                mark_file_processed(filename, conn)
